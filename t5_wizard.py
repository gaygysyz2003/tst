...
def available_actions(port):
    ...
    if port.kind == "client":
        ...
        if _is_up(port.state):
            ...
            acts.append(("ains", "Auto-in-service (8-hr soak, AINS)", False))
            acts.append(("up", "In service now (IS-NR)", False))
    return acts
...
def _apply_trunk(sess, port, key, freq=None):
    ...
    if key == "setfreq":
        ...
        return True
    if key == "ains":
        # auto-in-service: SOAK=32 (32 x 15min = 8 hrs) -> state OOS-AU,AINS,
        # promotes itself to IS-NR after the soak. Confirmed live on NE-77.
        _run(sess, "ED-OTU4C2::{}:{{c}}:::SOAK=32:IS,AINS".format(aid))
        return _verify(sess, "RTRV-OTU4C2::{}:{{c}}".format(aid), "AINS")
    if key == "up":
...
def guided_build(node, uid, pid, card):
    ...
    if trunk and not _is_up(trunk.state):
        ...
        plan.append(("trunk_ains",
                     "bring trunk {} in service (auto-in-service, 8-hr soak)".format(
                         trunk.port)))
    elif trunk:
    ...
    try:
        ...
        for k, desc in plan:
            ...
            if k == "trunk_freq":
                ...
                # line port -> auto-in-service, 8-hr soak (OOS-AU,AINS then IS-NR)
                ok, msg = _guided_run(
                    sess, "ED-OTU4C2::{}:{{c}}:::SOAK=32:IS,AINS".format(taid),
                    "RTRV-OTU4C2::{}:{{c}}".format(taid), "AINS", tolerate=("SAIN",))
