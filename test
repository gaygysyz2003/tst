...
Cisco 400G-XP-LC (T5) over TL1.

Unlike t5_menu.py (a fixed build-up / teardown recipe), this walks a *decision
tree whose options are computed from the node's live state*: it shows the card,
then each port, and for the chosen port offers ONLY the actions that are legal
right now.  Each action is applied and verified against the node before the next
set of options is shown -- because the commit changes what is possible next.

Design (confirmed on NE-77, 2026-07-03):
  * The NE enumerates NOTHING -- every RTRV returns current state only.  So the
    option CATALOG comes from the card model below (sourced from EPNM_OPTIONS.md)
    and the node supplies only CURRENT STATE + physical-optic presence + verify.
...
Reuses the TL1 layer from the engine (t5_provision / t5_configuration): the raw
socket TL1Session with ctag-matched reads (skips the async alarm storm), Node,
next_ctag, Tl1Error.
...
try:
    import t5_configuration as t5      # engine name on the sandbox
except ImportError:
...
# --- card model (from EPNM_OPTIONS.md; the NE does not enumerate these) --------
...
def _act_on_port(node, uid, pid, card: Card, port: Port):
    ...
    choice = _pick("  Select action (or 'q'): ", len(acts))
    if not choice:
        return
    key, label, destr = acts[choice - 1]
...
def run(uid, pid) -> int:
    ...
    while True:
        ...
        if len(cards) > 1:
            ...
            ci = _pick("Select card: ", len(cards)) or 1
            card = cards[ci - 1]

        while True:
            _show_card(card)
            print("\n  Pick a port to act on, 'r' refresh, 'b' back:")
